...
@respx.mock
def test_download_data_uses_content_type_when_no_url_extension(tmp_path: Path) -> None:
    landing = 'https://example.org/record'
    data_url = 'https://example.org/files/blob'
    ...
    assert result.unique_item_count == 1
    assert (tmp_path / 'blob.zip').read_bytes() == b'zipdata'
